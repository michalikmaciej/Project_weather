...
var page = function (p) {
        var c  = $('.frame');
        c.find('.city').text(p.obj.city);
        c.find('.temperature').html(p.obj.temp + ' &#8451;');
        c.find('.image >img').attr('src',p.obj.image);
        c.find('.description >p').text(p.obj.cond);
        c.find('.date').text(p.obj.date);
...
var start2 =function () {
    ...
    $.ajax({
        type: 'GET',
        url : "https://query.yahooapis.com/v1/public/yql?q=select location,item.condition,item.description from weather.forecast where woeid in (select woeid from geo.places(1) where "+txt()+")&format=json",
        dataType: 'json',
...
function execute(data) {
    var obj = data.query.results.channel;
    $.each(obj, function (index) {
        database.push(loadingData(obj[index]));
    });

    var rand = randomCites(database);
    var j = 0;
    var c1 = 0;
    var p1 = 10000;
    var one = function () {
