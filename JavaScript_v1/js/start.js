...
var loadingData = function (data) {
        ...
        var dateConverter = function (data) {
            var d = data.query.results.channel.item.condition.date.split(' ');
            var newDate = dayConverter(d[0])+' '+d[1]+'-'+monthConverter(d[2])+'-'+d[3];
            return newDate;
        }

        var conditionConverter = function (data) {
            var cond = data.query.results.channel.item.condition.code;
            var code = {
                0:'Tornado',
                1:'Burza Tropikalna',
                2:'Huragan',
        ...
        var getLink = function (data) {
            var desc = data.query.results.channel.item.description.split('<BR />')[11].split('"')[1];
        ...
        var getImg = function (data) {
            var img = data.query.results.channel.item.description.split('<BR />')[0].split('"')[1];
        ...
        var weather = function (data) {
            var w = {
                ...
                temp: faToCel(data.query.results.channel.item.condition.temp),
...
var start = function loadData(city,callback) {
    var url = "https://query.yahooapis.com/v1/public/yql?q=select location,item.condition,item.description from weather.forecast where woeid in (select woeid from geo.places(1) where text='"+city+"')&format=json";
...
function  execute(a, b) {
    ...
    if(a === null) {
        ...
        database.push(loadingData(data));
        i+= 1;
        ctr-=1;
    }
    if(a !== null){
    ...
    if(i<cities.length && ctr < cities.length*10){
        start(cities[i],execute);
    };
